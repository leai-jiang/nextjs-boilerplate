  ...
  return (
    <div className="relative w-[360px] h-[400px] flex items-center justify-center select-none">
      ...
      <img
        ...
        style={{ borderRadius: 12, boxShadow: '0 2px 12px 0 #297FCC22', clipPath: 'inset(0 10% 0 0)' }}
      />
      {/* 中间主图，带缩放动画 */}
      <img
        ...
        style={{ borderRadius: 16, boxShadow: '0 4px 24px 0 #297FCC44' }}
      />
      {/* 右侧预览，露出90%，带缩放动画 */}
      <img
        ...
        style={{ borderRadius: 12, boxShadow: '0 2px 12px 0 #297FCC22', clipPath: 'inset(0 0 0 10%)' }}
      />
      {/* 指示点 */}
      <div className="absolute bottom-[4px] left-0 right-0 flex justify-center gap-2">
        {certImages.map((_, i) => (
          <span key={i} className={`inline-block w-2 h-2 rounded-full ${i === index ? 'bg-[#297FCC]' : 'bg-gray-300'}`}></span>
        ))}
      </div>
    </div>
  );
